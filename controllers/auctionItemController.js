...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  // Define maximum auctions per day
  const MAX_AUCTIONS_PER_DAY = 10;

  // Get the current date (reset time to 00:00:00)
  const startOfDay = new Date();
  startOfDay.setHours(0, 0, 0, 0);

  // Count how many auctions the user has created today
  const todayAuctionsCount = await Auction.countDocuments({
    createdBy: req.user._id,
    createdAt: { $gte: startOfDay },
  });

  // Check if the user has reached the limit
  if (todayAuctionsCount >= MAX_AUCTIONS_PER_DAY) {
    return next(new ErrorHandler(`You can only create ${MAX_AUCTIONS_PER_DAY} auctions per day.`, 400));
  }
